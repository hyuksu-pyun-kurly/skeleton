#!/bin/bash
export PREFIX=${PREFIX:=DC0}
export MESOS_TASK_ID=${MESOS_TASK_ID:=.`echo $HOSTNAME | rev | cut -d "-" -f1 | rev`}
export DOCKER_ID="$PREFIX-`echo $MESOS_TASK_ID | cut -d "." -f2 | cut -c 1-13`"
export APP_NAME=${MARATHON_APP_LABEL_IO_VAMP_DEPLOYMENT:-skeleton-core-api}
...
export JAVA_OPTS="${JAVA_OPTS} \
                -javaagent:${PINPOINT_AGENT_JAR_PATH} \
...
exec java \
    ...
    -Dsentry.servername=skeleton-core-api \
    ...
    ${JAVA_OPTS} \
    ${JVM_OPT} \
    -jar /teseus/service/skeleton/skeleton-core-api-${VERSION}.jar
